...
class energyfunc(nn.Module): #give a penalty = sigma for each overlapping pair of monomers
    ...
    def get_poly(confdata):
        ...
        steps = list(map(randtovec,randvals))
#        pos_arr = np.cumsum(steps,axis=0)
        return steps

def randtovec(p):
    ph = (p[0])*2*np.pi
    th = torch.acos(1-2*(p[1])) #nf.dists module makes uniform distributions on [-0.5,0.5]
...
#prior = nf.distributions.UniformGaussian(dof,range(dof),torch.ones(dof))[0] #get random numbers uniformly distributed between -1/2 and 1/2
#
#num_layers = 20
#flow_layers = []
